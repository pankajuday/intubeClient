...
const CommentForm = () => {
  ...
  const { commentLoading, commentError } = useSelector(
    (state) => state.comment
  );
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      await dispatch(
        createVideoComment({ videoId, content: data.content.trim() })
      ).unwrap();
      reset();
      setIsFocused(false);
    } catch (error) {
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="flex gap-4">
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              {!errors.content && <div className="flex-1"></div>}
...
function VideoComments() {
  const { videoId } = useParams();
  const dispatch = useDispatch();
  const { commentData = [], commentLoading } = useSelector(
    (state) => state.comment
  );
      ...
      {commentLoading && (
        <div className="space-y-4">
          {[...Array(3)].map((_, index) => (
            <Card
              key={index}
              className="w-full bg-slate-950 border-slate-800 shadow-md overflow-hidden animate-pulse"
            >
              <CardContent className="p-5">
      ...
      {!commentLoading && commentData.length > 0 && (
        <div className="space-y-4 overflow-y-auto max-h-[800px] pr-1 scrollbar-thin scrollbar-thumb-orange-600/30 scrollbar-track-slate-900/30 hover:scrollbar-thumb-orange-600/50">
          {commentData.map((comment, index) => (
            ...
            >
              <VideoCommentCard data={comment} videoId={comment.video} />
            </div>
          ))}
        </div>
      )}
