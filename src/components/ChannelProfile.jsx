...
const ChannelProfile = () => {
  ...
  useEffect(() => {
    async function checkChannelIsSubscribed() {
      setIsSubscribed(
        await subscribedChannels.some((itm) => itm?._id === channelData?._id)
      );
    }
...
{/* subscription button */}
      <div className="relative  h-auto w-full flex items-center  ml-40 top-5">
        <div className="w-40 justify-center items-center flex ">
          {subscriptionIsLoading ? (
            <SpringLoader />
          ) : (
            <button
              className="border-2 rounded-sm cursor-pointer bg-slate-950 h-10 w-36 text-white outline-2 outline-slate-900 hover:bg-slate-700"
              onClick={toggleChannelSubscription}
            >
              {isSubscribed ? <span>unsubscribe</span> : <span>Subscribe</span>}
            </button>
          )}
        </div>
      </div>
